...
use crate::ai::{Provider, PARSE_RETRY, VALIDATION_RETRY};
...
fn test_key() -> String {
    std::env::var("OPENROUTER_API_KEY").expect("Set OPENROUTER_API_KEY explicitly to run a paid benchmark")
}
...
fn retry_delta(
    ...
    (after.0 + after.1)
        .saturating_sub(before.0 + before.1)
}
