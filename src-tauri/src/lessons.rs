...
pub(crate) fn generate(
    ...
    let turn =
        crate::execution::accept_coach(db, registry, snapshot, conversation, topic, c.revision)?;
...
pub(crate) fn control(
    ...
    match control {
        ...
        LessonControl::Practice => {
            ...
            let handoff = crate::execution::accept_opening(
                db,
                registry,
                snapshot,
                conversation,
...
fn stop_reviews(db: &Connection, lesson: &str) -> Result<()> {
    ...
    for turn in turns {
        ...
        crate::execution::refresh_turn(db, &turn)?;
    }
    Ok(())
}
pub(crate) fn choices(
    ...
    let c = snapshot
        ...
        .ok_or_else(|| invalid("Conversation not found."))?;
    let evidence =
        crate::progression::snapshot_db(db, registry, &snapshot.session_id, &c.language_id)?;
    let now: i64 = db.query_row("SELECT CAST(strftime('%s','now') AS INTEGER)", [], |r| {
        r.get(0)
    })?;
    let state = crate::learner_state::fold(registry, &evidence, now)?;
    let due: Vec<String> = state
        ...
        .collect();
    Ok(
        crate::openers::choices_db(db, registry, snapshot, conversation, &due)?
            .into_iter()
            .map(|(card, _)| card)
            .collect(),
    )
...
fn prose(value: &str, max: usize) -> Result<()> {
    ...
    crate::provider::validate_prose(value)
}
...
mod tests {
    ...
    fn quiz_grades_once_and_awards_only_one_bonus_xp_without_skill_evidence() {
        ...
        assert_eq!(
            crate::progression::snapshot(&store, "en").unwrap()["profile"]["xp"],
            0
        );
    ...
    fn reading_lessons_use_the_selected_native_language_not_assumed_english() {
        for native in ["fr", "ar"] {
            let (_dir, mut store, chat) = setup();
            store.connection.execute("UPDATE conversation_settings SET settings=json_set(settings,'$.explanationLanguage',?2) WHERE conversation_id=?1", params![chat, native]).unwrap();
            let revision = store.snapshot().unwrap().revision;
            let id = apply(
                &mut store,
                Action::GenerateLesson {
