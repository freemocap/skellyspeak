...
pub async fn transcribe_audio(
    ...
    if audio_base64.len() > 34 * 1024 * 1024 { return Err("Recording exceeds upload limit.".into()); }
    ...
    let _permit = crate::request_admission::shared().acquire(&endpoint.url).await?;
    let response = client
        .post(&endpoint.url)
        ...
        .send()
        .await
        .map_err(|e| format!("Transcription request failed: {}", e.without_url()))?;
