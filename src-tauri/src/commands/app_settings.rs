...
fn apply_settings(state: &AppState, mut settings: Settings, preserve_session: bool) -> Result<(), String> {
    ...
    if stored.custom_base_url.trim().trim_end_matches('/') != settings.custom_base_url.trim().trim_end_matches('/')
        && !stored.custom_api_key.is_empty()
        && settings.custom_api_key == settings::mask(&stored.custom_api_key) {
        return Err("Changing the custom server requires replacing or removing its saved credential.".into());
    }
