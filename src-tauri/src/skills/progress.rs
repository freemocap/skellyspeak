...
pub fn project(snapshot: &Snapshot, choices: Choices) -> Result<Profile, String> {
    ...
    for skill in snapshot.catalog.iter().filter(|s| s.kind == "skill") {
        ...
        for record in &snapshot.records {
            ...
            if record.input.suggestion || record.input.scaffold || record.input.revision { assisted.insert(source); } else { direct.insert(source); }
        }
        assisted.retain(|source| !direct.contains(source));
    ...
    Ok(Profile { rules_version: 1, xp: skills.iter().map(|s| s.xp).sum(), active_focus: choices.focus.clone().unwrap_or_else(|| recommended_focus.clone()), recommended_focus, choices, skills, branches })
}
