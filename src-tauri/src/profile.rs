...
fn summary(
    ...
    let (attempts,input_tokens,output_tokens,unknown_usage)=db.query_row(&format!("SELECT count(*),COALESCE(SUM(a.input_tokens),0),COALESCE(SUM(a.output_tokens),0),COALESCE(SUM(a.input_tokens IS NULL OR a.output_tokens IS NULL),0) FROM attempts a JOIN operations o ON o.id=a.operation_id JOIN turns t ON t.id=o.turn_id JOIN conversations c ON c.id=t.conversation_id JOIN relationships r ON r.id=c.relationship_id WHERE a.requested_model!='local' AND {predicate}"),params![language,partner],|r|Ok((r.get(0)?,r.get(1)?,r.get(2)?,r.get(3)?)))?;
    Ok(UsageSummary {
        id: id.into(),
        label: label.into(),
        conversations,
