  ...
  for (const segment of glossDisplayGroups(text, segments)) {
    ...
    const piece = interactive && annotations.length > 0
      ? <span className="wu saved-word" key={segment.start} data-source-start={segment.start} data-source-end={segment.end} onPointerEnter={event => { if (event.pointerType === 'mouse') { hoveredWord.current = event.currentTarget; setHovered(segment.start) } }} onPointerLeave={() => setHovered(null)}>
          ...
          {hovering && <span ref={helper} className="saved-word-help" dir="auto" popover="manual">{!autoTranslate && values('gloss', 'wg')}{!alwaysRomanize && values('romanization', 'wroman')}{!alwaysPronunciation && values('pronunciation', 'wpronunciation')}</span>}
          {(autoTranslate || open) && values('gloss', 'wg')}
          {(alwaysRomanize || open) && values('romanization', 'wroman')}
          {(alwaysPronunciation || open) && values('pronunciation', 'wpronunciation')}
        </span>
