...
export function SkillEvidenceRecord({ record, judgment, snapshot, children }: { record: SkillRecord; judgment: SkillJudgment; snapshot: SkillSnapshot; children: ReactNode }) {
  ...
  return <article className="skill-evidence-record"><header><strong>{skillIndex(snapshot).catalog.node(judgment.skill_id).label}</strong><span>{xp} XP credited</span></header><p>{state === 'complete' ? judgment.outcome.replaceAll('_', ' ') : state}</p>
