...
it('starts compact and restores resized height and collapse state across mounts', () => {
  ...
  let view = mount()
  expect(screen.getByLabelText('Resize coach panel')).toHaveAttribute('aria-valuenow', '64')
  ...
  fireEvent.keyDown(screen.getByLabelText('Resize coach panel'), { key: 'Enter' })
  expect(screen.getByLabelText('Coach panel')).toHaveStyle({ height: '160px' })
  view.unmount()
  bounds.mockRestore()
})
it('bounds resizing to leave room for the lesson and collapses at the minimum', () => {
  ...
  for (let i = 0; i < 5; i += 1) fireEvent.keyDown(divider, { key: 'ArrowDown' })
  expect(screen.getByLabelText('Coach panel')).toHaveClass('is-collapsed')
  expect(screen.getByLabelText('Message your coach')).toBeVisible()
  bounds.mockRestore()
})
...
it('dialog presentation stays expanded without reading or overwriting dock preferences', () => {
  ...
  expect(screen.getByLabelText('Coach panel')).not.toHaveClass('is-collapsed')
  expect(screen.getByLabelText('Coach panel').style.height).toBe('')
  expect(screen.queryByLabelText('Resize coach panel')).toBeNull()
  fireEvent.focus(screen.getByLabelText('Message your coach'))
  expect(localStorage.getItem('skellyspeak_coach_layout')).toBe(stored)
  view.unmount()
  mount()
  expect(screen.getByLabelText('Coach panel')).toHaveClass('is-collapsed')
})
