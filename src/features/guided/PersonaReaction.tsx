...
const reactions: Record<Reaction['kind'], { icon: string; label: string }> = {
  confused: { icon: '🤔?', label: 'Persona is unsure' },
  understood: { icon: '🙂', label: 'Persona understood' },
  curious: { icon: '🧐', label: 'Persona is curious' },
  surprised: { icon: '😮', label: 'Persona is surprised' },
  concerned: { icon: '😟', label: 'Persona is concerned' },
}
...
export function PersonaReaction({ reaction, error, message, reply, onEdit }: {
  ...
  const display = error ? { icon: '⚠', label: 'Persona reaction unavailable' } : reactions[reaction!.kind]
  return <>
    ...
    {open && <DetailDialog title="Persona’s interpretation" onClose={() => setOpen(false)}>
      <div className="reaction-details">
        ...
        <section className="reaction-exchange" aria-label="Conversation exchange">
          ...
          <div className="reaction-excerpt persona"><span>Persona reply</span><div className="msg bot" dir="auto"><TargetText text={reply} /></div></div>
        </section>
        ...
        <p className="lesson-meta">The persona’s self-report can be wrong. This is not a proficiency score or a measured emotion.</p>
