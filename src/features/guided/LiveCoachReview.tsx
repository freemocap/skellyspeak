...
export function LiveCoachReview({ turn, visible, onControl, nativeLanguageName, rtl }: {
  ...
  if (!turn) return <p className="lesson-meta">Your coach will follow the conversation here.</p>
  return <section ref={review} className="live-coach-review" aria-label="Conversation coaching">
    <h3>Your coach</h3>
    <CoachEntry source={null} decision={decision} feedback={turn.coach} error={turn.coachError} />
    {error && <p role="alert">{error}</p>}
    {decision?.shown && decision.exposedMove === decision.shown.move && decision.shown.move !== 'explicit' && <button type="button" className="lesson-action" onClick={() => { void onControl('show_answer').catch(reason => setError(nativeError(reason))) }}>Show answer</button>}
