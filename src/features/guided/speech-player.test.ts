...
it('owns the blob URL and releases playback resources once on stop', async () => {
  ...
  const player = playSpeechAudio({ status: 'ready', operationId: 'op', messageId: 'message', attemptId: 'attempt', mime: 'audio/mpeg', audioBase64: 'AA==' }, vi.fn(), vi.fn(), 0.65, 0.4)
  expect(media).toMatchObject({ playbackRate: 0.65, volume: 0.4, preservesPitch: true })
  await player.play()
