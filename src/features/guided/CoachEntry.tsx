...
export function CoachEntry({ decision, feedback, source, error }: {
  ...
  return <div className="coach-entry">
    ...
    {decision?.repairStatus === 'uncertain' && <p role="status">The coach could not confirm this revision yet.</p>}
    {decision?.fixed && <p className="coach-fixed" role="status"><span dir="auto">{decision.fixed}</span></p>}
    {shown && <section className="coach-card coach-card-help" aria-label="Coaching suggestion">
      {shown.move === 'explicit' ? <>
    ...
    {explanations.map((item, index) => <section className="coach-card coach-card-explanation" key={`${item.construct}:${index}`} aria-label="Language explanation">
