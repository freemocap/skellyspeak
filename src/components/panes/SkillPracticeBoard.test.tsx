...
it('browses all six domains without changing saved practice focus', () => {
  render(<Board />)
  const branch = within(screen.getByLabelText('Conversation skill map')).getByRole('button', { name: /Time, place & movement/ })
  ...
  expect(within(screen.getByLabelText('Conversation skill map')).queryByRole('button', { name: /Time, place & movement/ })).toBeNull()
  fireEvent.click(screen.getByRole('button', { name: 'Expand map' }))
  expect(within(screen.getByLabelText('Conversation skill map')).getByRole('button', { name: /Time, place & movement/ })).toHaveAttribute('aria-pressed', 'true')
})

 it('links all six cards to their map arms and remembers grid layout without changing selection', () => {
  const view = render(<Board />)
  fireEvent.click(screen.getByLabelText('Card display options'))
  fireEvent.click(screen.getByRole('button', { name: 'All areas' }))
  expect(view.container.querySelectorAll('.practice-card')).toHaveLength(6)
  ...
  expect(map.getByRole('button', { name: /Time, place & movement/ })).toHaveAttribute('aria-pressed', 'true')
  fireEvent.click(map.getByRole('button', { name: /Statements/ }))
