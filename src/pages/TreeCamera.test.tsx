...
it('keeps camera stable on refresh and restores the actual viewport on Back', async () => {
  ...
  view.rerender(<TreeCamera catalog={skillIndex(skillDemo).catalog} {...props} request={{ sequence: 1, target: 'situating', action: 'focus' }} />)
  await waitFor(() => expect(flow.fitView).toHaveBeenCalledTimes(2))
  expect(flow.fitView).toHaveBeenLastCalledWith(expect.objectContaining({ duration: 0, nodes: expect.arrayContaining([{ id: 'situating' }, { id: 'experience' }, { id: 'past_reference' }]) }))
  view.rerender(<TreeCamera catalog={skillIndex(skillDemo).catalog} {...props} request={{ sequence: 1, target: 'situating', action: 'focus' }} />)
  view.rerender(<TreeCamera catalog={skillIndex(skillDemo).catalog} {...props} request={{ sequence: 2, target: 'situating', action: 'back' }} />)
