...
it('keeps camera stable on refresh and restores the actual viewport on Back', async () => {
  ...
  const view = render(<TreeCamera {...props} request={{ sequence: 0, target: 'experience', action: 'whole' }} />)
  await waitFor(() => expect(flow.fitView).toHaveBeenCalledTimes(1))
  view.rerender(<TreeCamera {...props} request={{ sequence: 1, target: 'time', action: 'focus' }} />)
  ...
  view.rerender(<TreeCamera {...props} request={{ sequence: 1, target: 'time', action: 'focus' }} />)
  view.rerender(<TreeCamera {...props} request={{ sequence: 2, target: 'time', action: 'back' }} />)
