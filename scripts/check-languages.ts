...
import { validateLocales } from '../src/domain/language/i18n/messages.ts'
import type { Dict } from '../src/domain/language/i18n/dict.ts'

const root = path.resolve(import.meta.dirname, '..')
const localeDir = path.join(root, 'src/domain/language/i18n/locales')
...
const languages = fs.readdirSync(path.join(root, 'config/languages/languages')).filter(name => name.endsWith('.yaml')).map(file => {
  const text = fs.readFileSync(path.join(root, 'config/languages/languages', file), 'utf8')
  const id = /^id: "([^"\n]+)"$/m.exec(text)?.[1]
