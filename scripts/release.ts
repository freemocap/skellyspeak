...
//   node scripts/release.ts 1.0.0-rc.1     an explicit version
//   node scripts/release.ts minor --dry-run    say what would happen, change nothing
//   node scripts/release.ts patch --no-push    bump, commit and tag; push by hand
//
...
console.log('')
console.log('The Release workflow publishes the draft as Latest after all its jobs succeed.')
console.log('The separate iOS distribute workflow attaches the IPA and processes TestFlight:')
