...
impl TranscriptionManager {
    ...
    /// Transcribe and return the full transcript with word timestamps.
    ///
    /// Handy's `transcribe()` returns only cleaned text (dictation doesn't need
    /// timing). SkellySubs needs word timestamps for word-alignment, so this
    /// keeps the engine's full `Transcript`. Whisper (transcribe-cpp) only for
    /// now; the ONNX engines (Parakeet etc.) are added later.
    pub fn transcribe_detailed(&self, audio: Vec<f32>) -> Result<transcribe_cpp::Transcript> {
        self.touch_activity();

        if audio.is_empty() {
            return Ok(transcribe_cpp::Transcript::default());
        }
        ...
        let transcript = {
            ...
            let result = match &mut engine {
                LoadedEngine::TranscribeCpp(session) => {
                    let run_options = RunOptions {
                        timestamps: transcribe_cpp::TimestampKind::Word,
                        ..Default::default()
                    };
                    session
                        .run(&audio, &run_options)
                ...
                _ => Err(anyhow::anyhow!(
                    "word timestamps currently require a Whisper (transcribe-cpp) model"
                )),
            };
        ...
        self.maybe_unload_immediately("transcribe_detailed");
