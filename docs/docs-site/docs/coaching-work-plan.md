...
| **I · Integration** | Plan, contracts, sequencing, reviews, docs | `..`, `AGENTS.md`, `DESIGN.md`, `DATA-MODEL.md`, `BUILD-PLAN.md`, `workflow/reports/coaching-*` | everything |
...
- C also owns the bounded `src/platform/ipc/workspace.ts` change exposing the existing conversation pagination argument, with tests. B may register its new revision module in `native/src/lib.rs`; unrelated app lifecycle code stays outside this assignment.
...
- The coaching plan, this work plan, contracts, `references.bib`, a static reading view (`..`), and pointers in `AGENTS.md`, `DESIGN.md` and `BUILD-PLAN.md`.
