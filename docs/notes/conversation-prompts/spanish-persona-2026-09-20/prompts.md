# Exact prompts used in the Spanish identity comparison

Each block is the complete system message for an independent first-message call. No user message, topic, conversation history, or hidden persona was added. For no-persona calls, remove only `Identity: Lucía, an adult in Valencia. ` from the beginning. All other text is identical.

## Prompt-1: Short direct brief
...
## Prompt-2: Long Spanish relationship description
...
## Prompt-3: Explicit response contract
...
## Prompt-4: Demonstration examples
