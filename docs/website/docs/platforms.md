...
Provider routing is centralized in `native/src/settings.rs`:
...
`native/Cargo.toml` is the application-version source. Tauri, Android, and
...
cargo build --manifest-path native/Cargo.toml --bin skellyspeak
...
`native/target/debug/bundle/macos/SkellySpeak Dev.app` for native inspection.
...
The Android scaffold is in `native/gen/android`. Android uses the webview
...
`native/src/audio.rs` configures `AVAudioSession`; the app's microphone
usage description is in `native/Info.plist`.
...
`docs/website/` builds a Docusaurus site for
