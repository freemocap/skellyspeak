...
**Unaudited historical description:** this page contains obsolete module paths and
storage behavior. See the repository README for the current folder layout; the
current native implementation uses SQLite. The proposed
...
| `native/src/commands/guided/` | Turn execution, independent analysis/coach/observer work |
| `native/src/prompts/` | Prompt text and explicit instruction precedence |
| `native/src/ai.rs`, `sse.rs` | Provider requests, validation/retries and byte-safe SSE decoding |
| `native/src/trace.rs`, `trace_archive.rs`, `instruction.rs` | Captured request inputs, per-attempt records, bounded persistence and reconciliation |
| `native/src/turn_plan.rs` | Expected operation metadata for the graph and reconciliation |
| `native/src/lesson.rs`, `commands/lesson.rs` | Learner-owned choices, revision checks and change history |
| `native/src/observer.rs` | Bounded teaching plan/profile and atomic tutor-memory persistence |
| `native/src/credentials.rs`, `settings.rs` | Platform vault and public preferences |
| `native/src/conversation.rs`, `persistence.rs` | Chat files, soft deletion and atomic writes |
...
`src/contracts.ts`, which `native/src/bin/export-contracts.rs` writes;
