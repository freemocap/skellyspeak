...
`native/src/ontology.rs` declares two agents:
...
`native/src/turn_plan.rs` declares guided-turn operations, dependencies,
hydration behavior, conditional execution, and background work.
`native/src/graph.rs` derives the graph from that declaration. The graph
describes execution; `native/src/commands/guided/` executes it.
