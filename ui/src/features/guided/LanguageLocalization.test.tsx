...
function Editor() {
  ...
  return <><input aria-label={tr('Name')} value={draft} onChange={event => setDraft(event.target.value)} /><DifficultySelect value="beginner" saving={false} onChange={async () => {}} /><p>{languageLabel({ code: 'pt', name: 'Portuguese', endonym: 'Português' }, tr.locale)}</p><span>{tr.number(1234.5)}</span></>
}
describe('UI locale changes', () => {
  it('updates labels, options and formatting without discarding local input', async () => {
    const user = userEvent.setup()
    const { rerender } = render(<I18nProvider locale="de"><Editor /></I18nProvider>)
    ...
    rerender(<I18nProvider locale="pt"><Editor /></I18nProvider>)
