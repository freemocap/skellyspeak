...
export function LearnerModel({ target, onClose }: { target: string; onClose: () => void }) {
  ...
  return <DetailDialog title={tr("Your learning evidence")} onClose={onClose}>
    <section className="learner-model">
      ...
      {error && <div role="alert">{error}<button disabled={saving} onClick={() => refresh(value => value + 1)}>{tr("Reload evidence")}</button></div>}
      ...
      {data && <>
        ...
        {selected && <section aria-label={tr("Source evidence")}><h3>{data.evidence.catalog.find(node => node.id === selected)?.label}</h3><p>{tr("Excluding an attempt removes all its contributions to learning estimates and XP. You can restore it here.")}</p>{!records.length && <p>{tr("No recorded evidence for this skill.")}</p>}{records.map(record => {
          ...
          return <article className="practice-credit" key={record.attempt_id}><blockquote dir="auto">{record.source}</blockquote><p>{judgment.rationale}</p><small>{excluded ? tr("Excluded") : counted ? tr("Contributes to the selected estimates") : tr("Not used in the selected estimates")} · {new Date(record.at_secs * 1000).toLocaleDateString(tr.locale)} {tr(" · Conversation ")}{record.chat_id}{record.replaces_message_id !== null && tr(" · Revised message")}</small><button disabled={saving} onClick={() => void exclude(record.attempt_id)}>{excluded ? tr("Restore attempt") : tr("Exclude attempt")}</button></article>
        })}</section>}
        <details><summary>{tr("Calculation details")}</summary><p>{tr("View YAML and Save YAML export this language across all partners and varieties, including source observations, focus and exclusion choices, estimates and configuration hashes. Save YAML writes the file to Downloads.")}</p><p>{tr("Rating is on an internal logistic scale, not a percentage or language level. Uncertainty is a heuristic, not a statistical confidence interval. Review dates are heuristic. Time changes when practice is due; it does not remove your rating or XP. Repeated wording is counted conservatively. Assisted includes recorded coaching support and revisions.")}</p><p>{tr("Estimator version ")}{data.model.estimatorVersion} {tr(" · calculated ")}{new Date(data.model.asOfSecs * 1000).toLocaleString(tr.locale)}</p></details>
      </>}
    </section>
  </DetailDialog>
}
