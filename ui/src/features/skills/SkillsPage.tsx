...
import { TreeCamera, type CameraRequest } from './overview/TreeCamera'
...
function SkillNode({ data }: NodeProps<Node<SkillNodeData>>) {
  ...
  return <button className={`tree-node ${item.kind} ${picked ? 'selected' : ''}`} style={{ '--node-color': item.color, '--node-scale': data.scale } as CSSProperties} aria-label={`${item.code} ${item.label}, ${status}${focus ? ', focus' : ''}`} aria-pressed={picked} onClick={() => onPick(item.id)}>
    <Handle type="target" position={target} /><span className="tree-node-code">{item.code}<span>{focus ? tr("◆ FOCUS") : picked ? tr("SELECTED") : ''}</span></span>
...
function marks(progress: SkillProgress, tr: ReturnType<typeof useI18n>): string {
  return progress.star ? tr('★ {count} successes', { count: progress.successes }) : `${'✓'.repeat(progress.successes)}${'○'.repeat(3 - progress.successes)} ${progress.successes}/3`
}
...
export function SkillTreeView({ languageTag, snapshot, demonstration, refresh, save, saving, onPractice }: { languageTag?: string; snapshot: SkillSnapshot; demonstration: boolean; refresh: () => void; save: (choices: ProfileChoices) => Promise<void>; saving: boolean; onPractice: () => void }) {
  ...
  const status = useCallback((node: TreeNode) => {
    if (node.kind === 'root') return tr('{mark} {count} conversations · {xp} XP', { mark: snapshot.conversation_count ? '●' : '○', count: snapshot.conversation_count, xp: profile.xp })
    const p = profile.skills.find((s) => s.skill_id === node.id)
    if (p) return `${marks(p, tr)}${p.assisted ? tr(' · {count} assisted', { count: p.assisted }) : ''}`
  ...
  }, [profile, snapshot.conversation_count, descendants, tr])
  ...
  const edges = useMemo<Edge[]>(() => visible.filter((node) => node.parent !== null).map((node) => ({ id: `${node.parent}-${node.id}`, source: node.parent!, target: node.id, type: layout === 'radial' ? 'straight' : 'smoothstep', style: { stroke: selected === node.id || selected === node.parent ? 'var(--ink)' : node.color, strokeWidth: selected === node.id || selected === node.parent ? 3 : 2, vectorEffect: 'non-scaling-stroke' } })), [layout, selected, visible])
    ...
    <div className={`tree-workspace ${detailOpen ? 'details-open' : 'details-closed'}`}>
      ...
      <aside className={`tree-inspector ${detailOpen ? 'expanded' : ''}`} aria-label={tr("Selected node")} style={{ '--node-color': item.color, '--node-scale': scale(item.id) } as CSSProperties}>
        ...
        <div className="tree-inspector-body">
          <div className="tree-detail-actions"><button disabled={!canBack} onClick={back}>{tr("← Back")}</button><button onClick={wholeTree}>{tr("Whole tree")}</button><button aria-label={tr("Close node details")} onClick={() => setDetailOpen(false)}>×</button></div>
          {!detailModal && detailContent}
          <button className="lesson-action" onClick={() => setDetailModal(true)}>{tr("Open full details")}</button>
          <ProgressRules />
          <p className="lesson-meta">{tr("Profile revision ")}{profile.choices.revision} {tr(" · rules ")}{profile.rules_version} · {snapshot.target}</p>
          <details className="tree-method"><summary>{tr("Assessment activity · ")}{snapshot.records.filter((r) => r.status === 'pending').length} {tr(" pending · ")}{failures.length} {tr(" failed")}</summary><button className="tree-refresh" onClick={refresh}>{tr("Refresh")}</button>{failures.map((r) => <p key={r.attempt_id}>{r.chat_id}/{r.message_id}: {r.error}</p>)}</details>
        </div>
      </aside>
    </div>
    </div></div>
