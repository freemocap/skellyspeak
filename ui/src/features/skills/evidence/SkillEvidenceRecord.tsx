...
export function SkillEvidenceRecord({ languageTag, record, judgment, snapshot, children }: { languageTag?: string; record: SkillRecord; judgment: SkillJudgment; snapshot: SkillSnapshot; children: ReactNode }) {
  ...
  return <article className="skill-evidence-record"><header><strong>{skillIndex(snapshot).catalog.node(judgment.skill_id).label}</strong><span>{xp} {tr(" XP credited")}</span></header><p>{state === 'complete' ? judgment.outcome.replaceAll('_', ' ') : state}</p>
    {judgment.quotes.map((quote, i) => <blockquote key={i} lang={languageTag}><TargetText text={quote} /></blockquote>)}<p>{judgment.rationale}</p><p className="lesson-meta">{assistance} {tr(" · external assistance unknown")}</p>
