...
it('requires explicit typed confirmation and allows cancel without erasing', () => {
  render(<FactoryReset />)
  fireEvent.click(screen.getByRole('button', { name: 'Delete my data and close' }))
  const erase = screen.getByRole('button', { name: 'Delete all data and close' })
...
it('sends confirmation once and exposes native failures for retry', async () => {
  ...
  fireEvent.click(screen.getByRole('button', { name: 'Delete my data and close' }))
  fireEvent.change(screen.getByLabelText(/Type DELETE/), { target: { value: 'DELETE' } })
  fireEvent.click(screen.getByRole('button', { name: 'Delete all data and close' }))
  ...
  expect(screen.getByRole('button', { name: 'Delete all data and close' })).toBeEnabled()
})
