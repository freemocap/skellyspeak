...
it.each([
  ...
  render(<SettingsAccess onBusyChange={vi.fn()} onChanged={vi.fn()} />)
  const key = await screen.findByLabelText(label, { selector: 'input' })
  fireEvent.focus(key)
...
it('keeps model selection out of hosted access', async () => {
  ...
  render(<SettingsAccess onBusyChange={vi.fn()} onChanged={vi.fn()} />)
  expect(await screen.findByRole('tab', { name: 'Hosted sign-in' })).toHaveAttribute('aria-selected', 'true')
...
it('saves an unsaved URL default once before checking the returned revision', async () => {
  ...
  native.mockImplementation(async (command, args) => {
    ...
    if (command === 'check_access') { expect(args.expectedRevision).toBe(8); return verified }
    throw new Error(command)
  })
...
it.each([
  ...
  expect(input).toHaveAttribute('placeholder', 'saved***12345')
  expect(input).toHaveValue('')
  fireEvent.focus(input)
  fireEvent.change(input, { target: { value: ' \t abcde-hidden-middle-vwxyz \n ' } })
  expect(input).toHaveValue('abcde-hidden-middle-vwxyz')
  expect(screen.getByLabelText(`${label} preview`)).toHaveTextContent('abcde***vwxyz')
  expect(screen.queryByText('abcde-hidden-middle-vwxyz')).toBeNull()
  fireEvent.change(input, { target: { value: 'tiny' } })
  expect(screen.getByLabelText(`${label} preview`)).toHaveTextContent('***')
  fireEvent.click(screen.getByRole('button', { name: 'Discard changes' }))
  expect(screen.queryByLabelText(`${label} preview`)).toBeNull()
  expect(input).toHaveAttribute('placeholder', 'saved***12345')
})
...
it('shows server and token verification and replaces it with a failed check', async () => {
  ...
  native.mockImplementation(async (command: string) => {
    ...
    if (command === 'check_access') {
      if (rejected) throw new Error('The custom server rejected the session token.')
      return verified
    }
    throw new Error(command)
  })
  render(<SettingsAccess onBusyChange={vi.fn()} onChanged={vi.fn()} />)
  fireEvent.click(await screen.findByRole('button', { name: 'Check connection' }))
  await waitFor(() => expect(screen.getAllByText('Accepted')).toHaveLength(4))
  expect(useConnectionHealth.getState().routes.custom?.status).toBe('connected')
  rejected = true
