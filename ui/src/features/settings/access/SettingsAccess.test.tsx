...
it('shows the same model editor on hosted access', async () => {
  native.mockImplementation(async (command: string) => {
  ...
  expect(screen.getByLabelText('Standard model')).toHaveValue('fixture-standard')
  expect(screen.getByLabelText('Fast model')).toHaveValue('fixture-fast')
  expect(screen.getByLabelText('Transcription model')).toHaveValue('fixture-transcription')
})
