...
const mocks = vi.hoisted(() => ({ inspect: vi.fn(), getSettings: vi.fn(), save: vi.fn() }))
...
const current = { target_language:'arabic', target_variety:'arabic-levantine', native_language:'english', native_variety:'english-united-states' } as Settings
function report(id='arabic-levantine'): LanguageInspection {
  return {
    ...
    language:{id:'arabic',name:'Arabic',nativeName:'العربية',languageTag:'ar',defaultVariety:'arabic-levantine',fontScale:1.5,direction:'rtl',romanization:'arabic:ala-lc-arabic',varieties:[]},
...
beforeEach(() => {
  ...
  useSettingsStore.setState({...useSettingsStore.getInitialState(),settings:current,save:mocks.save})
})
...
it('saves the inspected variety only after explicit selection', async () => {
  ...
  expect(mocks.save).not.toHaveBeenCalled()
  fireEvent.click(screen.getByRole('button',{name:'Use this language and variety'}))
...
it('shows load errors with retry and keeps failed saves open', async () => {
  ...
  expect(screen.getByRole('button',{name:'Use this language and variety'})).toBeDisabled()
  fireEvent.click(screen.getByRole('button',{name:'Retry'}))
  await screen.findByText('kitāb')
  fireEvent.change(screen.getByLabelText('Variety'),{target:{value:'arabic-modern-standard'}})
  await waitFor(() => expect(screen.getByRole('button',{name:'Use this language and variety'})).toBeEnabled())
...
it('omits empty language-specific sections rather than showing generic filler', async () => {
  mocks.inspect.mockResolvedValue({ ...report(), rules: [], goals: [] })
  render(<LanguageBrowser onClose={vi.fn()} />)
  await screen.findByText('kitāb')
