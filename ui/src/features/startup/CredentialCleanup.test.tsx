...
it('keeps cleanup failure visible until native retry confirms success', async () => {
  ...
  await waitFor(() => expect(screen.getByRole('button', { name: 'Retry credential cleanup' })).toBeEnabled())
...
it('preserves recovery after a rejected retry', async () => {
  ...
  expect(screen.getByRole('button', { name: 'Retry credential cleanup' })).toBeEnabled()
})
