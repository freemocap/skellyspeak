  ...
  const chatComposer = (
        <div className="composer" ref={composer}>
          ...
          <ComposerInput waveform={mic.recording && mic.waveSource ? <WaveformStrip source={mic.waveSource} height={44} timelineSeconds={10} /> : null} micShortcut={settings?.shortcuts.mic} input={input} available={isTauri} sending={sending}
            ...
            transcriptionWarning={targetLanguage?.transcriptionLanguage === null && connection ? tr("The {model} transcription model has no language code for {language}; output may be unreliable.", { model: connection.audio.transcription.model, language: targetLanguageName }) : undefined}
