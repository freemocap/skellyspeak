...
export function PersonaPicker({ choices, currentId, busy, onSelect, onEdit, onCreate, status }: {
  status?: string
  ...
  return <div className="persona-picker" ref={panel}>
    <button type="button" className="persona-picker-toggle" aria-haspopup="menu" aria-expanded={open} disabled={busy}
      ...
      {current ? <><PersonaAvatar symbol={current.symbol} /><span className="partner-identity"><strong>{current.name}</strong>{status && <span className="partner-status">{status}</span>}</span></> : <span>{tr("No persona")}</span>}
