...
it('shows per-skill XP in conversation and continues beyond the first milestone', async () => {
  ...
  snapshot.profile.skills.find(s=>s.skill_id==='referent')!.xp=52
  render(<SkillEvidenceContext value={{snapshot,error:null}}><PracticeContext value={{chatId:'chat',selected:'referent',selectionVersion:0,select:vi.fn()}}><ConversationMap /></PracticeContext></SkillEvidenceContext>)
  ...
  expect(document.querySelectorAll('[data-reward-skill]')).toHaveLength(45)
})
