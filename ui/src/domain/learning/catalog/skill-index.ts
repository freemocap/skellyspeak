...
import { requireCatalogVersion, type SkillJudgment, type SkillRecord, type SkillSnapshot } from '../evidence/skills'
...
export function createSkillCatalog(catalog: TreeNode[]) {
  ...
  const painted = catalog.map(item => ({ ...item, color: item.kind === 'root' ? 'var(--ink)' : domainColors(domain(item.id).id).bright }))
