...
function sample(): SkillSnapshot {
  ...
  const record: SkillRecord = { attempt_id: 'a', session_id: 's', turn_id: 1, message_id: 1, replaces_message_id: null, construct_registry_hash: 'fixture-registry', mapping_error: null, support_step: null, chat_id: 'chat', learner_id: snapshot.learner_id, target: snapshot.target, native: 'en', source: 'That cup is blue.', input: { modality: 'text', suggestion: false, scaffold: false, revision: false }, at_secs: 100, model: 'test', provider_mode: 'custom', catalog_version: SKILL_CATALOG_VERSION, prompt_version: 'v1', status: 'complete', assessment: { judgments: [first, second].map(skill => ({ skill_id: skill.skill_id, outcome: 'demonstrated', quotes: ['cup'], rationale: 'Recorded judgment' })) }, error: null }
...
it('adds quiz XP without inventing skill evidence and scopes it to its conversation', async () => {
  const { conversationEvidence } = await import('./skills')
  const snapshot = structuredClone(skillDemo)
