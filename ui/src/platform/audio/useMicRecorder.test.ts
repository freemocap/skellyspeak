...
it('publishes new take status while a spectrogram request is still outstanding', async () => {
  let takes: { recordingId: string; state: string }[] = []
  let finishSpectrum!: () => void
  invoke.mockImplementation(async (command: string) => {
    if (command === 'mic_listen_start') return { recordingId: 'live', samplesPerSecond: 689 }
    if (command === 'mic_listen_status') return { recordingId: 'live', listening: true, queued: takes.length, processing: false, completed: 0, speaking: false, failure: null, takes }
    if (command === 'mic_listen_spectrogram') return new Promise(resolve => { finishSpectrum = () => resolve(null) })
    if (command === 'mic_wave') return []
    if (command === 'mic_cancel') return
    throw new Error(`Unexpected native command: ${command}`)
  })
  ...
  await waitFor(() => expect(result.current.listeningStatus?.takes).toEqual(takes))
