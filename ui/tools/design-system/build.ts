...
for (const role of ["sans", "serif", "mono"])
  tokens.type.families[role] = lookup(`--font-${role}`, "light");
...
// Keep font faces, including script unicode ranges, identical to the application.
...
if (stageAt && !errors.length) {
  ...
  for (const file of readdirSync(FONTS))
    if (/\.(?:ttf|woff2)$/.test(file) || file.endsWith("-OFL.txt") || file.endsWith("-NOTICE.txt") || file === "sources.json")
      cpSync(join(FONTS, file), join(project, "fonts", file));
