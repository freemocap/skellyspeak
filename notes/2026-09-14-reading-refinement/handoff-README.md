...
- **Mystery partner — Persona tab (new):** a third study-pane tab, purple ink (`#7d5bd4`/`#0e5c53` for confirmed). Body is a list of rows, one per hidden attribute (job, personality, where-from, age, interests): each row shows either the real value + "✓ revealed", or "?" (locked, dashed border), or "? · guessed correctly · +N XP" with a **separate** "Reveal" text-button. **Decided:** a correct guess pays ordinary XP immediately (same pool the rest of the app uses — an earlier draft tried a separate "curiosity" currency to keep it out of language evidence; dropped as overthinking a detail that doesn't matter) but does *not* auto-populate the value — the player must tap Reveal separately. Ship the third state per field (`hidden | guessed_unrevealed | revealed`) — confirmed wanted, add it to the persona schema. Below the rows: a dashed amber system-nudge card (dismissible, appears after ~8 turns, suggest a per-conversation dismissal flag) and a persistent **"I think I know them"** button. Tapping it (or the nudge) opens a small multiple-choice popover — real value + 2–3 plausible distractors — scoped to one field at a time. **Decided:** ship that popover as a `DetailDialog` (the app's existing modal primitive) for v1 — no new positioning logic — and revisit a lighter anchored popover later if it feels heavy in practice.
