...
Reports Firestore account usage. Writes require an explicit per-user limit command.
    ...
    uv run --project server python -m server.operations.stats              # today
    uv run --project server python -m server.operations.stats --days 7     # the last week
    uv run --project server python -m server.operations.stats --emails     # show addresses instead of masking them

Changing a daily limit requires --user:

    uv run --project server python -m server.operations.stats --user me@x.com --set-limit-usd 5
...
def apply_limit(db: firestore.Client, user_id: str, dollars: float) -> None:
    ...
    print("  and mirror it into server/deployment/cloudbuild.yaml or the next deploy reverts it.\n")
