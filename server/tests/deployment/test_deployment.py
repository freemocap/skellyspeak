...
import pytest
from server.deployment.retention import COLLECTIONS
from server.deployment.deploy_candidate import deploy, DeploymentError
from server.deployment.verify_revision import inspect_revision, inspect_traffic
...
def test_cloud_failures_identify_operation_without_echoing_details(monkeypatch, detail, category):
    import subprocess
    from server.deployment.deploy_candidate import gcloud
...
def test_all_local_runtime_imports_are_copied_into_container():
    ...
    runtime = {path.relative_to(root).as_posix() for path in (root / 'app').rglob('*.py')}
