import json
import pytest
from server.deployment.retention import COLLECTIONS, RetentionError, ensure_retention
...
def test_cloud_upload_is_an_explicit_allowlist():
    ...
    lines = (Path(__file__).parents[3] / ".gcloudignore").read_text().splitlines()
    assert lines[0] == "*"
    assert lines[1:3] == ["!server/", "server/*"]
    assert not any("*" in line for line in lines if line.startswith("!"))
    assert "server/app/*" in lines
    assert "server/deployment/*" in lines
    assert "!server/deployment/retention.py" in lines
    assert "!server/app/admission/work_admission.py" in lines
    assert "!server/app/inference/model_routing.py" in lines
    assert not any("local" in line or "test_" in line for line in lines)


def test_retention_failure_prevents_any_revision_change():
    from server.deployment.deploy_candidate import deploy, DeploymentError
    calls = []
    def call(args):
        calls.append(args)
