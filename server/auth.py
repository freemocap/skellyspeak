...
def issue_session_token(*, user_id: str, signing_key: str, now: int | None = None) -> str:
...
def read_session_token(token: str, *, signing_key: str) -> str:
    """Return the user id a session token attests to, or raise."""
    try:
        claims = jwt.decode(
            token,
            signing_key,
