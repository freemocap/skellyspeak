...
pub fn implicated(captured: &Value) -> Result<Vec<Value>> {
    ...
    let items = assessment["items"].as_array().ok_or_else(|| fail("missing Jev decisions"))?;
    let criteria = captured["skillCriteria"].as_array().ok_or_else(|| fail("missing criteria"))?;
    items.iter().filter(|item| matches!(item["outcome"].as_str(), Some("demonstrated" | "partial")))
        .map(|item| criteria.iter().find(|c| c["id"] == item["construct"]).cloned().ok_or_else(|| fail("unknown implicated skill"))).collect()
}
pub fn schema(captured: &Value) -> Result<Value> {
    let ids: Vec<_> = implicated(captured)?.iter().map(|c| c["id"].clone()).collect();
    Ok(json!({"type":"object","additionalProperties":false,"required":["items"],"properties":{"items":{"type":"array","minItems":ids.len(),"maxItems":ids.len(),"items":{"type":"object","additionalProperties":false,"required":["construct","quote"],"properties":{"construct":{"type":"string","enum":ids},"quote":{"type":"string","minLength":1,"maxLength":300}}}}}}))
}
...
pub fn prompt(db: &Connection, turn: &str, captured: &Value) -> Result<Vec<PromptMessage>> {
    let source = db.query_row("SELECT text FROM messages WHERE turn_id=?1 AND role='user'", [turn], |r| r.get(0))?;
    prompt_for_source(source, captured)
}
#[derive(Deserialize)]
#[serde(deny_unknown_fields)]
struct Evidence { items: Vec<Span> }
#[derive(Deserialize)]
#[serde(deny_unknown_fields)]
struct Span { construct: String, quote: String }
...
fn validate_source(source: &str, captured: &Value, output: &Completion) -> Result<Value> {
    if output.finish_reason != "stop" || output.text.len() > 32000 { return Err(fail("incomplete or oversized output")); }
    let parsed: Evidence = serde_json::from_str(&output.text).map_err(|_| fail("invalid fields"))?;
    let expected = implicated(captured)?;
    if parsed.items.len() != expected.len() { return Err(fail("skill coverage mismatch")); }
    let mut result = captured["skillDecisions"].clone();
    let mut seen = std::collections::HashSet::new();
    for span in parsed.items {
        if !expected.iter().any(|c| c["id"] == span.construct) || !seen.insert(span.construct.clone()) { return Err(fail("unknown or duplicate skill")); }
        if span.quote.trim().is_empty() || span.quote.chars().count() > 300 || source.char_indices().filter(|(start, _)| source[*start..].starts_with(&span.quote)).count() != 1 { return Err(fail("quote must bind uniquely to current learner text")); }
        let item = result["items"].as_array_mut().unwrap().iter_mut().find(|i| i["construct"] == span.construct).unwrap();
...
pub fn publish(db: &Connection, turn: &str, result: &Value) -> Result<()> {
    let attempt: String = db.query_row("SELECT json_extract(context,'$.skillDecisionsAttempt') FROM turns WHERE id=?1", [turn], |r| r.get(0))?;
...
mod tests {
    use super::*;
    fn captured() -> Value { json!({"skillCriteria":[{"id":"question","criterion":"Request information"}],"skillDecisions":{"adapter":"jev_choice","model":"typesafe/jev-1.13","items":[{"construct":"question","outcome":"demonstrated","evidenceKind":"whole_message","scores":{"full":0.9},"answer":{"choice":"demonstrated"}}]}}) }
    fn output(items: Value) -> Completion { Completion { diagnostics: None, text: json!({"items":items}).to_string(), finish_reason:"stop".into(), actual_model:"selected-fast".into(), provider_id:"test".into(), input_tokens:Some(10), output_tokens:Some(5) } }
    #[test]
    fn unicode_source_quotes_preserve_assessor_decisions() {
        let result = validate_source("Hola 👋 ¿cómo estás?", &captured(), &output(json!([{"construct":"question","quote":"¿cómo estás?"}]))).unwrap();
        assert_eq!(result["model"], "typesafe/jev-1.13");
        assert_eq!(result["evidenceModel"], "selected-fast");
        assert_eq!(result["items"][0]["scores"], captured()["skillDecisions"]["items"][0]["scores"]);
    ...
    fn rejects_missing_duplicate_unknown_invented_and_ambiguous_evidence() {
        for items in [json!([]),json!([{"construct":"other","quote":"Hola"}]),json!([{"construct":"question","quote":"invented"}]),json!([{"construct":"question","quote":""}]),json!([{"construct":"question","quote":"Hola"},{"construct":"question","quote":"Hola"}]),json!([{"construct":"question","quote":"Hola","outcome":"partial"}])] {
            assert!(validate_source("Hola", &captured(), &output(items)).is_err());
        }
        assert!(validate_source("Hola Hola", &captured(), &output(json!([{"construct":"question","quote":"Hola"}]))).is_err());
    ...
    fn prompt_contains_only_implicated_criteria_and_current_message() {
        ...
        input["skillCriteria"].as_array_mut().unwrap().push(json!({"id":"absent","criterion":"Never sent"}));
        input["skillDecisions"]["items"].as_array_mut().unwrap().push(json!({"construct":"absent","outcome":"not_observed"}));
        ...
        assert_eq!(schema(&input).unwrap()["properties"]["items"]["maxItems"],1);
    }
}
