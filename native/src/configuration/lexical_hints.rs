...
pub(super) struct LexicalHints(Vec<String>);
impl LexicalHints {
    pub(super) fn new(fragments: &[String]) -> Self {
        Self(words(&fragments.join(" ")))
    }
...
mod tests {
    ...
    fn phrases_match_contiguous_words_with_unicode_case_accents_and_punctuation() {
        ...
        assert!(LexicalHints::new(&["D’FHÁG".into()]).contains("d'fhág"));
    }
}
