    ...
    fn codes(value: &Value, limited: &mut bool, quota: &mut bool, depth: usize) {
        ...
        if let Some(items) = value.as_array() {
            for item in items.iter().take(64) {
                codes(item, limited, quota, depth + 1);
            }
            return;
        }
        for key in ["status", "code", "reason"] {
            let code = &value[key];
            *limited |= code.as_u64() == Some(429)
                || matches!(
                    code.as_str(),
                    Some("429" | "rate_limit" | "rate_limit_exceeded" | "too_many_requests")
                );
            *quota |= matches!(
                code.as_str(),
                Some(
        ...
        for key in [
            ...
            if let Some(child) = value.get(key) {
                codes(child, limited, quota, depth + 1);
            }
        }
    }
...
mod tests {
    ...
    fn classifies_http_and_nested_completion_limits_but_not_quota_or_content() {
        ...
        assert!(detected(
            &json!({"response":{"choices":[{"error":{"code":429}}]}})
        ));
        assert!(!detected(
            &json!({"status":429,"response":{"error":{"code":"insufficient_quota"}}})
        ));
