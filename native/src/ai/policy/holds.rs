...
pub fn recover(db: &Connection, id: &str, generation: &str) -> Result<()> {
    ...
    if hold
        ...
        .as_ref()
        .and_then(|r| r.retry_at)
        .is_some_and(|at| at > crate::ai::policy::refusal::now())
    {
        return Err(AppError::new(
            ErrorCode::AdmissionHeld,
...
mod tests {
    ...
    fn hold_capacity_fails_closed_and_spending_pause_is_not_downgraded() {
        ...
        record(
            ...
            &AppError::new(ErrorCode::Provider, "Rate limited.").with_refusal(
                crate::ai::policy::refusal::classify(Some("INGRESS_RATE_LIMIT"), Some(60), None),
            ),
        )
        .unwrap();
        assert!(matches!(
    ...
    async fn refusal_arriving_during_audio_capacity_wait_prevents_admission() {
        let (db, target) = setup();
        let admission = crate::ai::policy::admission::Admission::new();
        let running: Vec<_> = (0..crate::ai::policy::admission::NETWORK_CAPACITY)
