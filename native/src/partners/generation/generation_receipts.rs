...
use crate::model::*;
use crate::partners::generation::Request;
...
pub fn record_retry(store: &Store, request: &Request, error: &AppError) -> Result<()> {
    ...
    if store.connection.execute(
        "UPDATE persona_generation_attempts SET diagnostics=?2 WHERE id=?1 AND state='running'",
...
mod tests {
    ...
    fn provider_refusal_and_held_authority_survive_restart_until_explicit_recovery() {
        ...
        let error =
            crate::partners::generation::accept_completion(&mut store, &captured, &provider_result)
                .unwrap_err();
