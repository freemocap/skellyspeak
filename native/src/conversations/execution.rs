...
mod tests {
    ...
    fn audit_lesson_review_drops_old_exchanges_to_fit_serialized_budget() {
        ...
        let prompt = crate::learning::lessons::prompt(
            &store.connection,
            &current,
            "lesson_review",
            &captured,
        ...
        while let Some(work) = store.dispatch().unwrap() {
            ...
            let expected = match kind.as_str() {
                ...
                "user_word_gloss" | "persona_word_gloss" => {
                    crate::ai::connections::model_routing::OSS
                }
    ...
    fn focus_is_frozen_and_reaches_partner_and_both_coach_prompts() {
        ...
        let block =
            crate::conversations::conversation_prompt::focus_block(&captured["practiceFocus"])
                .unwrap();
        assert!(
        ...
        for kind in [
            ...
            let prompt =
                crate::learning::coaching::prompt(&store.connection, &turn, kind, &captured)
                    .unwrap();
            assert!(prompt[0].content.contains(&block));
            let mut no_focus = captured.clone();
            no_focus["practiceFocus"] = serde_json::Value::Null;
            let prompt =
                crate::learning::coaching::prompt(&store.connection, &turn, kind, &no_focus)
                    .unwrap();
