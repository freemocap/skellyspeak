...
fn accept_turn(
    ...
    if coach {
        ...
        let mut support = saved.iter().map(|s| -> Result<serde_json::Value> { let v: serde_json::Value=serde_json::from_str(s)?; Ok(serde_json::json!({"feedback":v["conversation_feedback"],"assistance":v["reply_assistance"],"explanations":v["reply_explanations"]})) }).collect::<Result<Vec<_>>>()?;
