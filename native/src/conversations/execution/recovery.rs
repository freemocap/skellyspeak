...
impl Store {
    ...
    pub(crate) fn shelve_jev_assessment(&self) -> Result<()> {
        ...
        for turn in &turns {
            let error = AppError::new(ErrorCode::Validation, "Jev assessment is disabled. Retry this turn to assess with the selected Fast chat model.");
        ...
        if changed > 0 || !turns.is_empty() { bump(&tx)?; }
        tx.commit()?;
        Ok(())
    }
}
