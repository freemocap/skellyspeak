...
fn r1_queue_reserves_translation_before_accepting_send() {
    ...
    let count: i64 = store.connection.query_row("SELECT count(*) FROM operations WHERE kind IN ('skill_assessment','persona_reply','reply_translation','persona_word_gloss','conversation_feedback','reply_assistance','reply_explanations','user_word_gloss','user_translation')", [], |r| r.get(0)).unwrap();
