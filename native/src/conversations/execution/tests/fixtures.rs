...
pub(super) fn isolate_coaching(store: &mut Store) {
    ...
    store.connection.execute("DELETE FROM operations WHERE kind IN ('coach_feedback','coach_suggestions','coach_reaction','skill_assessment','conversation_feedback','reply_brief','reply_assistance','reply_explanations') AND state='waiting_dependencies'", []).unwrap();
}
