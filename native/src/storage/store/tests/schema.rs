...
fn generation_receipt_schema_enforces_identity_state_and_nonnegative_usage() {
    ...
    let insert = "INSERT INTO generation_attempts(id,attempt_id,operation_id,language_id,route,requested_model,profile_revision,state,input_tokens,output_tokens) VALUES(?1,?2,?3,'spanish','custom','fixture',1,?4,?5,?6)";
    store
        .connection
        .execute(
            insert,
