...
const GENERATION_SCHEMA: &str = include_str!("schemas/generation_schema.sql");
    ...
    pub fn execute(&mut self, command: Command) -> Result<Receipt> {
        ...
        let entity_id = match command.action {
            ...
            Action::RevealMystery {
                ...
                crate::partners::mystery::reveal(&tx, &snapshot, &conversation_id, field)?;
                conversation_scope = Some(conversation_id.clone());
                conversation_id
            }
            Action::DismissMysteryNudge { conversation_id } => {
                crate::partners::mystery::owner(&snapshot, &conversation_id)?;
                crate::partners::mystery::dismiss(&tx, &conversation_id)?;
                conversation_scope = Some(conversation_id.clone());
                conversation_id
            }
            Action::AnswerLessonQuiz {
                ...
                crate::learning::lessons::check(&snapshot, &conversation_id, snapshot.revision)?;
            ...
            Action::AskLessonCoach {
                ...
                crate::learning::lessons::check(&snapshot, &conversation_id, expected_revision)?;
                let lesson = crate::learning::lessons::owned(&tx, &conversation_id, &lesson_id)?;
                ...
                crate::learning::lessons::attach_question(&tx, &turn, &lesson)?;
                conversation_scope = Some(conversation_id);
                turn
            }
            ...
            Action::SendMessage {
                ...
                input.scaffold |=
                    crate::learning::lessons::capture_exposure(&tx, &conversation_id, &turn_id)?;
            ...
            Action::RequestMessageSpeech { message_id } => {
                ...
                let operation =
                    crate::conversations::execution::request_speech(&tx, &message_id, resident)?;
                conversation_scope = Some(tx.query_row(
            ...
            Action::CancelMessageSpeech { operation_id } => {
                let operation = crate::conversations::execution::cancel_speech(&tx, &operation_id)?;
            ...
            Action::RequestSuggestions { message_id } => {
                let (conversation, operation) =
                    crate::conversations::execution::request_suggestions(&tx, &message_id)?;
                conversation_scope = Some(conversation);
                operation
            }
            Action::RetryGloss { operation_id } => {
                conversation_scope = Some(crate::conversations::execution::retry_gloss(
                    &tx,
                    &operation_id,
                )?);
                operation_id
            ...
            Action::ControlTurn { turn_id, control } => {
                conversation_scope = Some(crate::conversations::execution::control_turn(
                    &tx, &turn_id, control,
                )?);
                turn_id
            }
            ...
            Action::RecoverAiAccess {
                ...
                crate::ai::policy::holds::recover(&tx, &hold_id, &expected_generation)?;
                hold_id
            }
            ...
            Action::UpdatePersona {
                ...
                crate::partners::mystery::validate_edit(&tx, &persona_id, &details)?;
                tx.execute(
