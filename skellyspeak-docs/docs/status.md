...
PRE-A1 activates a true-beginner survival mode in `prompts::partner::learner_block`: one new
...
- `localStorage.skellyspeak_target` can go stale vs. actual settings. — **open**
...
| A2 | **The language overlay was injected twice into the reply prompt** — the template interpolated both `{overlay}` and `{directives}`, and `directives` already begins with the overlay. `prompts::partner::reply_prompt` now takes `directives` only, the same single vehicle the mechanics and scaffolds prompts already used; the unused `target_language` param went with it. | med | ✅ |
...
| A4 | `Settings.prompt_overrides` is persisted but never read — no registry in `prompts/`, no UI. **Do not delete it** (reversing the initial call): it is the configuration surface for the agent workbench in [Observability](./observability), where agent id == prompt id. Build the registry in observability bite 2. | med | ⬜ → folded into Observability |
