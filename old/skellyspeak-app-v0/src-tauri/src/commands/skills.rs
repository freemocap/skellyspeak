...
fn snapshot(config: &Path, target: &str) -> Result<LearnerSnapshot, String> {
    ...
    let profile = skills::progress::project(&evidence, crate::learner::load(config, target)?)?;
...
#[tauri::command]
pub fn save_language_profile(app: tauri::AppHandle, state: State<'_, AppState>, target: String, expected_revision: u64, choices: crate::learner::LanguageProfile) -> Result<LearnerSnapshot, String> {
    let _context = state.context_epoch.lock().expect("context lock poisoned");
    ...
    let current = crate::learner::load(&state.config_dir, &target)?;
    ...
    crate::learner::save(&state.config_dir, &target, expected_revision, choices)?;
