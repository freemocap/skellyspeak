...
use crate::learner::LOCAL_LEARNER;
...
pub fn snapshot(config: &Path, target: &str) -> Result<Snapshot, String> {
    let mut snapshot = Snapshot { catalog: catalog()?, catalog_version: CATALOG_VERSION, learner_id: LOCAL_LEARNER.into(), target: target.into(), conversation_count: 0, records: vec![] };
    ...
    for pair in std::fs::read_dir(directory).map_err(|e| e.to_string())? {
        ...
        for chat in crate::conversation::list_chats(&pair.path())? {
            ...
            for mut record in read(&path)?.records {
                if record.learner_id != LOCAL_LEARNER || record.target != target || record.chat_id != chat.id || crate::conversation::pair_key(&record.target, &record.native) != pair_name {
...
mod tests {
    ...
    fn record(source: &str) -> EvidenceRecord {
        EvidenceRecord {
            ...
            replaces_message_id: None, chat_id: "chat-a".into(), learner_id: LOCAL_LEARNER.into(), target: "es-ES".into(), native: "en".into(),
    ...
    fn progress_cannot_credit_another_target_language() {
        ...
        assert_eq!(progress::project(&view, crate::learner::LanguageProfile::initial("es-ES")).unwrap().xp, 10);
        view.records[0].target = "ar".into();
        assert!(progress::project(&view, crate::learner::LanguageProfile::initial("es-ES")).err().unwrap().contains("another language"));
    ...
    fn progression_is_deduplicated_assistance_aware_and_reversible() {
        ...
        let choices = crate::learner::LanguageProfile::initial("es-ES");
    ...
    fn historical_rubrics_remain_readable_without_new_skill_credit() {
        ...
        assert_eq!(progress::project(&view, crate::learner::LanguageProfile::initial("es-ES")).unwrap().xp, 0);
