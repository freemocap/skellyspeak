...
beforeEach(() => {
  ...
  disarmGreeting()
  backend.invoke.mockImplementation(async (command: string) => command === 'get_conversation_practice' ? { revision: 0, difficulty: 'beginner' } : command === 'get_conversation_partner'
...
describe('the suggestions panel', () => {
  ...
  it('says what is folded away, so the steering is never invisible', async () => {
    ...
    await waitFor(() => expect(screen.getByLabelText('Current conversation settings')).toHaveTextContent(/Beginner/))
...
it('applies quick reading toggles to both chat and the open Coach tray', async () => {
  ...
  backend.loadConversation.mockResolvedValue({ id: 'chat-1', turns: [greeting] })
  backend.invoke.mockImplementation(async (command: string, args: { text: string }) => command === 'get_conversation_practice' ? { revision: 0, difficulty: 'beginner' } : command === 'annotate_text' ? { tokens: args.text.split(/\s+/).filter(Boolean).map(text => ({ text, gloss: text, pronunciation: text === 'Buenos' ? 'bweh-nos' : text === 'días.' ? 'dee-as' : text, romanization: null, pos: null, notable: false })) } : '')
  ...
  expect(await screen.findByText('bweh-nos')).toBeVisible()
  expect(screen.getByText('dee-as')).toBeVisible()
...
it('waits for conversation settings before greeting', async () => {
  backend.loadConversation.mockResolvedValue({ id: 'practice-chat', turns: [] })
  let finish: (value: { revision: number; difficulty: string }) => void = () => { throw new Error('No request') }
  backend.invoke.mockImplementation(async (command: string) => command === 'get_conversation_practice' ? new Promise(resolve => { finish = resolve }) : '')
  render(<GuidedPage languagePicker={null} mobileSurface="chat" onMobileSurfaceChange={vi.fn()} active />)
  await waitFor(() => expect(backend.invoke).toHaveBeenCalledWith('get_conversation_practice', { target: 'es-ES', native: 'en', chatId: 'practice-chat' }))
  expect(backend.rawInvoke).not.toHaveBeenCalled()
  await act(async () => { finish({ revision: 0, difficulty: 'zero' }) })
  await waitFor(() => expect(backend.rawInvoke).toHaveBeenCalledWith('guided_turn', expect.objectContaining({ level: 'zero', chatId: 'practice-chat' })))
  expect(backend.loadConversation).toHaveBeenCalledTimes(1)
})
