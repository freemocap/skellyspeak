...
Execution plan, September 9, 2026. **Refactor authorized; target architecture
is being implemented.** Conversation difficulty and language progress profiles are implemented
in Rust; the remaining ownership and product work below is planned. The
...
| Language profile | Language-scoped progress, focus, evidence projections and proficiency history |
...
| Conversation | Explicit learner, relationship and language context; persistent practice settings, ordered source messages and its own flower projection |
...
| Canonical language and preference scope | One progress profile per language; variety-tagged evidence; difficulty and practice settings belong to persistent conversations; relationship defaults are a separate explicit design | Profile schema |
...
profile's choices; each conversation restores its selected difficulty; changing explanation language or dialect cannot lose language progress;
