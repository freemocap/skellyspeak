...
pub async fn coach_ask(
    ...
    let (epoch, stored, pair, coach_dir, mut thread, plan, profile, lesson, context, skill_context) = {
        ...
        crate::conversation_practice::require_difficulty(&coach_dir, level)?;
        ...
        let skill_profile = crate::skills::progress::project(&evidence, crate::learner::load(&state.config_dir, &stored.target_language)?)?;
