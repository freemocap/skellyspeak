...
pub fn load_conversation(
    ...
    if loaded.turns.as_array().is_some_and(Vec::is_empty) {
        crate::conversation_practice::initialize(&chat)?;
    } else {
        crate::conversation_practice::load(&chat)?;
    }
    info!(
...
#[tauri::command]
pub fn get_conversation_practice(state: State<'_, AppState>, target: String, native: String, chat_id: String) -> Result<crate::conversation_practice::Practice, String> {
    let _context = state.context_epoch.lock().expect("context lock poisoned");
    let chat = practice_chat(&state, &target, &native, &chat_id)?;
    crate::conversation_practice::load(&chat)
}

fn practice_chat(state: &AppState, target: &str, native: &str, chat_id: &str) -> Result<std::path::PathBuf, String> {
    require_current_pair(state, target, native)?;
    let pair = conversation::pair_dir(&state.config_dir, target, native)?;
    if conversation::current_chat(&pair)?.as_deref() != Some(chat_id) { return Err("The conversation changed. Reopen its settings.".into()); }
    conversation::chat_dir(&pair, chat_id)
}

#[tauri::command]
pub fn save_conversation_practice(state: State<'_, AppState>, target: String, native: String, chat_id: String, expected_revision: u64, difficulty: crate::prompts::difficulty::Difficulty) -> Result<crate::conversation_practice::Practice, String> {
    let mut epoch = state.context_epoch.lock().expect("context lock poisoned");
    let chat = practice_chat(&state, &target, &native, &chat_id)?;
    let practice = crate::conversation_practice::save(&chat, expected_revision, difficulty)?;
    *epoch += 1;
    Ok(practice)
}
