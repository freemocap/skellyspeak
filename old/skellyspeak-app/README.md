...
Practice difficulty is saved by Rust for each persistent conversation. Two Spanish
conversations can have different settings; reopening a conversation restores its
selection. Guided replies, coach answers, suggestions and topic explanations validate
that conversation’s setting. Language profiles aggregate progress across conversations.
Difficulty is a choice, not a proficiency score.
