...
import { STEER_LEVELS, STEER_TOPICS, useSteering, usePersonaPreference } from '../hooks/useSteering'
...
export default function GuidedPage({
  ...
  const personaPreference = usePersonaPreference()
  const [greetingPending, setGreetingPending] = useState<string | null>(null)
  ...
  const {
    ...
    settings,
    persona: personaPreference.persona,
    sending,
    setHistoryOpen,
    greet: () => greetRef.current(),
    resetView,
  })
  const steer = { ...useSteering(settings?.target_language ?? null, settings?.native_language ?? null, currentChatId), ...personaPreference }
  useEffect(() => { if (steer.error) setError(steer.error) }, [steer.error])
  ...
  const requestTurn = useCallback(
    async (body: { message?: string; greeting?: boolean; steering?: string; replacesMessageId?: number; inputEvidence?: InputEvidence }) => {
      if (!steer.ready) {
        setError('Wait for conversation settings to finish loading or saving.')
        return
      }
    ...
    [autoSpeak, onBubbleTap, speakReply, steer.level, steer.topic, steer.ready, chatIdRef]
  )

  async function send(text: string) {
    const message = text.trim()
    if (!message || sending || !steer.ready) return
  ...
  greetRef.current = () => setGreetingPending(JSON.stringify(chatIdRef.current))
  useEffect(() => {
    if (!greetingPending || greetingPending !== JSON.stringify(chatIdRef.current) || !steer.ready) return
    setGreetingPending(null)
    void requestTurn({ greeting: true })
  }, [greetingPending, currentChatId, steer.ready, requestTurn])
  ...
  const scaffolds = useScaffolds({
    settingsLoaded: settings !== null && steer.ready,
    scope: `${settings?.target_language}:${settings?.native_language}:${currentChatId}`,
    level: steer.level,
    topic: steer.topic,
        ...
        <div className="composer" ref={composer}>
          ...
          >
            ...
            <button
              type="submit"
              className="send"
              disabled={sending || !steer.ready || !input.trim()}
              aria-label="Send"
            >
              ↑
            </button>
    ...
    >
        ...
        <div className="chat-head" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <div className="chat-config" ref={settingsPanel}>
            ...
            {settingsOpen && <div id="chat-settings" className="scaffold-groups chat-config-panel" role="region" aria-label="Chat settings">
                {steer.error && <button type="button" onClick={steer.reload}>Retry conversation settings</button>}
                <fieldset className="conversation-controls" disabled={sending || !steer.ready}>
                <div className="steer-row">
                  <select
                    className="steer-select"
                    value={steer.level}
                    onChange={(e) => { void steer.setLevel(e.target.value).catch((failure: unknown) => setError(String(failure))) }}
                    aria-label="Learner level"
                    title="Learner level — steers every prompt"
                  >
                    {!steer.level && <option value="" disabled>Loading conversation settings…</option>}
                    {STEER_LEVELS.map((l) => (
          ...
          <button type="button" className="new-chat" aria-label="New chat"
            ...
            disabled={!settings || sending || !steer.ready}
      ...
      >
        ...
        {currentChatId && <CoachAnalysisPanel
          ...
          conversationBusy={sending || !steer.ready}
          plan={plan}
          profile={profile}
